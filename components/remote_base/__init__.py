...
# Nikai
NikaiData, NikaiBinarySensor, NikaiTrigger, NikaiAction, NikaiDumper = declare_protocol(
    "Nikai"
)
...
@register_binary_sensor("nikai", NikaiBinarySensor, NIKAI_SCHEMA)
def nikai_binary_sensor(var, config):
    cg.add(
        var.set_data(
            cg.StructInitializer(
                NikaiData,
...
async def nikai_action(var, config, args):
    template_ = await cg.templatable(config[CONF_DATA], args, cg.uint32)
    cg.add(var.set_data(template_))
